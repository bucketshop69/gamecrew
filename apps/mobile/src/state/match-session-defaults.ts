import { engineFramesPollIntervalMs, fetchEngineFrames } from '../api/gamecrew';
 ...
 * has no runtime import of `../api/gamecrew` and can be exercised directly
 * by the mobile package's plain-Node test runner (see match-session.ts's
 * header comment). Only the React hook layer (`use-playback-engine.ts`)
 * imports this file.
 */
